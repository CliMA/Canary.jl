...
# ```
# where $q^{(e)}_N=\sum_{i=1}^{(N+1)^{dim}} \psi_i(\mathbf{x}) q_i$ and  $\mathbf{Q}^{(e)}_N=\sum_{i=1}^{(N+1)^{dim}} \psi_i(\mathbf{x}) \mathbf{Q}_i$ are the finite dimensional expansion with basis functions $\psi(\mathbf{x})$ and $\Psi$ is the block diagonal tensor with blocks comprised of $\psi$ as follows
# ```math
...
# ```math
# M^{(e)}_{i,j} Q^{(e)}_j = \mathbf{F}_{i,j} q^{(*,e)}_j - \widetilde{\mathbf{D}}^{(e)} q^{(e)}_j \; \; (6)
# ```
#
# Next, integrating Eq. (3) by parts gives a similar form to Eq. (6) as follows
# ```math
# M^{(e)}_{i,j} \frac{\partial^2 q^{(e)}_j}{\partial x^2} = \mathbf{F}_{i,j} Q^{(*,e)}_j - \widetilde{\mathbf{D}}^{(e)} Q^{(e)}_j \; \; (7)
# ```
#
# Since we use the average flux for both $q$ and $Q$, we can reuse the same functions to construct $\mathbf{F}$ and $\widetilde{\mathbf{D}}$.
#-
# ## Running the Code
#
# You can run this code in one of two ways:
# 1) using the REPL (Read-Eval-Print-Loop) window type: julia> include("new_burger1d.jl")  or
# 2) from a terminal prompt type: mpirun -np # julia new_burger1d.jl where # is the number of MPI ranks.
#
#-
# ## Commented Program
#
...
# {{{ -----------------------------------CPU Kernels
# {{{ Volume RHS for 1D
function volumerhs!(::Val{1}, ::Val{N}, rhs::Array, Q, gradQ, vgeo, D, elems, visc) where N
    DFloat = eltype(Q)
    Nq = N + 1
    nelem = size(Q)[end]

    Q = reshape(Q, Nq, _nstate, nelem)
    gradQ = reshape(gradQ, Nq, _nstate, nelem)
...
# Flux RHS for 1D
function fluxrhs!(::Val{1}, ::Val{N}, rhs::Array,  Q, gradQ, sgeo, elems, vmapM, vmapP, elemtobndy, visc) where N
    DFloat = eltype(Q)
    Np = (N+1)
    Nfp = 1
    nface = 2
    ...
    @inbounds for e in elems
        for f = 1:nface
            for n = 1:Nfp
                ...
                UM = Q[vidM, _U, eM]
                UxM = gradQ[vidM, _U, eM]
                bc = elemtobndy[f, e]
                UP = zero(eltype(Q))
                if bc == 0
                    UP = Q[vidP, _U, eP]
                    UxP = gradQ[vidP, _U, eP]
                elseif bc == 1
                    UnM = nxM * UM
                    UxnM = nxM * UxM
                    UP = UM - 2 * UnM * nxM
                ...
                #Left Fluxes
                fluxUM_x = 0.5*UM^2 - visc*UxM

                #Right Fluxes
                fluxUP_x = 0.5*UP^2 - visc*UxP
...
# {{{ Update solution
function updatesolution!(::Val{dim}, ::Val{N}, rhs, Q, vgeo, elems, rka, rkb, dt, visc) where {dim, N}
...
function update_gradQ!(::Val{dim}, ::Val{N}, Q, vgeo, elems) where {dim, N}
...
# {{{ Compute RHS
function computerhs!(::Val{dim}, ::Val{N}, mesh, sendreq, recvreq, sendQ, recvQ, d_sendelems, d_sendQ, d_recvQ, d_QC, d_QL,
                     d_gradQC, d_gradQL, d_vgeoC, d_vgeoL, d_sgeo, d_vmapM, d_vmapP, d_elemtobndy, d_rhsC, d_rhsL, d_D,
                     visc, mpicomm; ArrType=ArrType) where {dim, N}
    ...
    volumeQ!(Val(dim), Val(N), d_gradQC, d_QC, d_vgeoC, d_D, mesh.realelems)

    # Receive q Data
    receivedata!(Val(dim), Val(N), mesh, recvreq, recvQ, d_recvQ, d_QL)

    # face Grad q computation
    fluxQ!(Val(dim), Val(N), d_gradQL, d_QL, d_sgeo, mesh.realelems, d_vmapM, d_vmapP, d_elemtobndy)

    # Construct Q=Grad q pointwise
    update_gradQ!(Val(dim), Val(N), d_gradQL, d_vgeoL, mesh.realelems)
    ...
    # volume RHS computation: f_total=f_inv - nu*f_visc
    volumerhs!(Val(dim), Val(N), d_rhsC, d_QC, d_gradQC, d_vgeoC, d_D, mesh.realelems, visc)

    # Receive Grad q Data
    receivedata!(Val(dim), Val(N), mesh, recvreq, recvQ, d_recvQ, d_gradQL)

    # face RHS computation: f_total=f_inv - nu*f_visc
    fluxrhs!(Val(dim), Val(N), d_rhsL, d_QL, d_gradQL, d_sgeo, mesh.realelems, d_vmapM, d_vmapP, d_elemtobndy, visc)
...
function lowstorageRK(::Val{dim}, ::Val{N}, mesh, vgeo, sgeo, Q, rhs, D,
    ...
    for step = 1:nsteps
        for s = 1:length(RKA)
            #Compute RHS => d_rhsL
            computerhs!(Val(dim), Val(N), mesh, sendreq, recvreq, sendQ, recvQ, d_sendelems, d_sendQ, d_recvQ, d_QC, d_QL,
                     d_gradQC, d_gradQL, d_vgeoC, d_vgeoL, d_sgeo, d_vmapM, d_vmapP, d_elemtobndy, d_rhsC, d_rhsL, d_D,
                     visc, mpicomm; ArrType=ArrType)
...
# {{{ RK loop
function lowstorageRK_v0(::Val{dim}, ::Val{N}, mesh, vgeo, sgeo, Q, rhs, D,
                      dt, nsteps, tout, vmapM, vmapP, mpicomm, visc;
                      ArrType=ArrType, plotstep=0) where {dim, N}
    DFloat = eltype(Q)
    mpirank = MPI.Comm_rank(mpicomm)
...
function burger(::Val{dim}, ::Val{N}, mpicomm, ic, mesh, tend, visc;
    ...
    writemesh(@sprintf("viz/burger%dD_%s_rank_%04d_step_%05d",
              ...
              fields=(("h", U),("U",U)),realelems=mesh.realelems)
    #Plot initial condition using Plots
    plot_initial=plot(X,U,xlabel="x",ylabel="u(x,t)",title="1D Burgers Equation - Initial Time",legend=false,lw=3)
    ...
    writemesh(@sprintf("viz/burger%dD_%s_rank_%04d_step_%05d",
              ...
              fields=(("h", U),("U",U)),realelems=mesh.realelems)
    #plot final solution using Plots
    plot_final=plot(X,U,xlabel="x",ylabel="u(x,t)",title="1D Burgers Equation - Final Time",legend=false,lw=3)
    display(plot(plot_initial,plot_final,layout=(2,1),legend=false))

    #Compute Diagnostics: Energy and Error
