...
"""
    ...
    """
function kessler_element(rho, t, qv, qc, qr, p, z, rainnc, rainncv, dt, icol)
    ...
    for k = 1:nz_inside
        # autoconversion and accretion: (Klemp and Wilhelmson (1978) eq. (2.13))
        factorn = 1.0 / (1.0 + 2.2*dt*max(0.0, rain_off_flg*qr[k])^0.875)
        qrprod = qc[k] * (1.0 - factorn) &
            + factorn*0.001*dt*max(qc[k] - 0.001, 0.0)
        qrprod = rain_off_flg*0.1*qrprod #only 10% of rain production, SM
        
        rcgs = rain_off_flg*0.001*rho[k]

        qc[k] = max(qc[k] - qrprod, 0.0)
        qr[k] = rain_off_flg*(qr[k] + prod[k] - qr[k])
        qr[k] = rain_off_flg*max(qr[k] + qrprod,0.0)
