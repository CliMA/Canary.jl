...
function BrickTopology(
    ...
    topology = partition(mpicomm, topology...)
    origsendorder = topology[5]
    topology = connectmesh(mpicomm, topology[1:4]...)
...
function cubedshellmesh(Ne; part = 1, numparts = 1)
    ...
    elemlocal = linearpartition(prod(globalnelems), part, numparts)
